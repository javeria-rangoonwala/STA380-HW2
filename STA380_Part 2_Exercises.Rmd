...
d1 = incoming %>%
  group_by(Origin) %>%
  summarize(canc_pct = sum(Cancelled==1)/n())%>% 
...
d2 = incoming %>%
  group_by(DayOfWeek) %>%
  summarize(canc_day_pct = sum(Cancelled==1)/n()) 
d2
...
d3 = incoming %>%
  ...
  summarize(time_canc_pct = sum(Cancelled==1)/n())
d3
...
```{r}
x = table(UniqueCarrier)
z = table(Cancelled, UniqueCarrier)

y = table(can_count$Cancelled, can_count$UniqueCarrier)
```
```{r}
y
barplot(y, main="Cancelled Flights per Carrier", xlab = "Unique Carrier", ylab = "Number of Cancelled Flights", las = 2)
```
...
norm_canc = airdata %>%
  ...
  summarize(canc_norm = sum(Cancelled==1)/n())
norm_canc
